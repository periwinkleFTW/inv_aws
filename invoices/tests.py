...
class CustomerTests(TestCase):
    ...
    def test_invoice_listing(self):
        self.assertEqual(f'{self.invoice1.customer}', 'Harry Potter')
        self.assertEqual(f'{self.invoice1.valid}', 'True')
        # self.assertIsInstance(f'{self.invoice1.expiration_date}', datetime)
        self.assertEqual(f'{self.invoice1.status}', 'Unpaid')
        self.assertEqual(f'{self.invoice1.tax_rate}', '12')
        self.assertEqual(f'{self.invoice1.discount}', '1')

        self.assertEqual(f'{self.invoice2.customer}', 'Frodo Baggins')
        self.assertEqual(f'{self.invoice2.valid}', 'False')
        # self.assertEqual(f'{self.invoice2.expiration_date}', datetime())
        self.assertEqual(f'{self.invoice2.status}', 'Paid')
        self.assertEqual(f'{self.invoice2.tax_rate}', '0')
        self.assertEqual(f'{self.invoice2.discount}', '30')

    def test_invoice_list_view_for_logged_in_user(self):
        ...
        self.assertTemplateUsed(response, 'invoices/invoice-table.html')
    ...
    def test_customer_list_view_for_logged_out_user(self):
        ...
        self.assertRedirects(
            response, '%s?next=/invoices/list' % (reverse('account_login')))
        response = self.client.get(
            '%s?next=/invoices/list' % (reverse('account_login')))
